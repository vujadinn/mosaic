...
const extractColorFromEvent = (ev: string): string | undefined => {
  return ev || undefined;
};

export const fetcher: (url: RequestInfo, options: {}) => Promise<any> = (
  url,
  options
) => fetch(url, options).then((r) => r.json());

export const makeSetColor =
  (data: MosaicTile[], refreshTiles: TileRefresher) =>
  async (ev: string, x: number, y: number): Promise<void> => {
